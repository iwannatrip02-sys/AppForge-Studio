...
private let logger = Logger(subsystem: "com.appforgestudio", category: "PincelRenderer")
class StrokeRenderer {
    ...
    init(device: MTLDevice) {
        ...
        guard let library = device.makeDefaultLibrary() else {
            logger.info("Warning: No Metal library found. Stroke rendering disabled.")
        ...
        guard let vertexFn = library.makeFunction(name: "strokeVertex"),
            ...
            logger.info("Warning: Stroke shaders not found in Metal library.")
            self.pipelineState = nil
            return
        }
        ...
        do {
            ...
            logger.info("Failed to create stroke pipeline: \(error)")
            self.pipelineState = nil
        }
    }
