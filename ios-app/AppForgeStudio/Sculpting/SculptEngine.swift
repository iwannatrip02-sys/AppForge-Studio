...
enum DeformerType: String, Codable, CaseIterable {
    case inflate, pinch, smooth, crease, grab, flatten, twist, move
}
...
class SculptEngine {
    ...
    func apply(at point: SculptPoint, to vertices: inout [Vertex]) {
        ...
        for i in 0..<vertices.count {
            applyDeformer(at: point, to: &vertices[i])
        }
        if symmetryEnabled {
            ...
            for i in 0..<vertices.count {
                applyDeformer(at: symPoint, to: &vertices[i])
            }
        }
    }
    
    private func applyDeformer(at point: SculptPoint, to vertex: inout Vertex) {
        ...
        case .grab:
            vertex.position += dir * influence * 0.3
        case .flatten:
