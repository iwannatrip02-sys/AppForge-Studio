...
class ShellEngine {
    ...
    func computeShell(faceIndex: Int, thickness: Float, mesh: inout Mesh) -> Bool {
        ...
        // Find all triangles coplanar with the reference face
        ...
        // Compute per-vertex average normals
        ...
        // Create inner shell by offsetting face vertices inward
        ...
        // Inner shell triangles (flipped winding)
        for tri in faceTriangles {
            let i0 = Int(mesh.indices[tri])
            let i1 = Int(mesh.indices[tri + 1])
            let i2 = Int(mesh.indices[tri + 2])
        ...
        // Build wall triangles along boundary edges of the selected face
